...
hparams = tf.contrib.training.HParams(
	...
	#Mel spectrogram
	n_fft = 1024,#1024 at first , 2048 suggested by llq, #Extra window size is filled with 0 paddings to match this parameter
	...
	#Residual postnet
	postnet_conv_layers = 5, #number of postnet convolutional layers
	postnet_conv_kernel_size = (5, ), #size of postnet convolution filters for each layer
	postnet_conv_firters = 512, #number of postnet convolution filters for each layer
	...
	tacotron_batch_size = 32, #number of training samples on each training steps for each gpu
	...
	tacotron_decay_steps = 70000, #Determines the learning rate decay slope (UNDER TEST)
	...
	tacotron_reg_weight = 2e-5, #regularization weight (for L2 regularization)
	...
	tacotron_teacher_forcing_final_ratio = 0.33,#llq:0., #final teacher forcing ratio. Relevant if mode='scheduled'
	tacotron_teacher_forcing_start_decay = 30000,#llq:30000, #starting point of teacher forcing ratio decay. Relevant if mode='scheduled'
	tacotron_teacher_forcing_decay_steps = 20000,#280000, #Determines the teacher forcing ratio decay slope. Relevant if mode='scheduled'
	tacotron_teacher_forcing_decay_alpha = 1e-5,#llq:0., #teacher forcing ratio decay rate. Relevant if mode='scheduled'
	###########################################################################################################################################
	# multi speaker
	use_multispeaker=False,
	speaker_num=300,
	speaker_dim=128,

	###################### style encoder
	ref_enc_filters=[32, 32, 64, 64, 128, 128],
	ref_enc_conv_kernel=(3, 3),
	ref_enc_conv_stride=(2, 2),
	ref_enc_rnn_units=128,
	ref_enc_average_pooling=False,

	use_style_encoder=True,
	style_encoder_type='vae',
	...
	# vae
	z_latent_dim=32,
	no_sampling_in_syn=False,
	vae_warming_up=20000,
	kl_x = 60000,
	kl_k = 0.0002,
	update_kl_before = 100,
	update_kl_after = 400,
	###########################################################################################################################################
