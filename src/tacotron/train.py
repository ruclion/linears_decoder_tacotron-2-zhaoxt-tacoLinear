...
#os.environ["CUDA_VISIBLE_DEVICES"]="2,3"#2,4常用
...
def add_train_stats(model, hparams):
	with tf.variable_scope('stats') as scope:
		...
		if hparams.use_style_encoder and (hparams.style_encoder_type=='vae' or hparams.style_encoder_type=='cvae'):
...
def model_train_mode(args, feeder, hparams, global_step):
	with tf.variable_scope('Tacotron_model', reuse=tf.AUTO_REUSE) as scope:
		...
		if hparams.predict_linear:
			model.initialize(feeder.inputs, feeder.speaker, feeder.input_lengths, feeder.mel_targets, feeder.token_targets, linear_targets=feeder.linear_targets,
				targets_lengths=feeder.targets_lengths, global_step=global_step,
				is_training=True, split_infos=feeder.split_infos, reference_mels=feeder.reference_mels, reference_lengths = feeder.reference_lengths,Lf0=feeder.Lf0)
		else:
			model.initialize(feeder.inputs, feeder.speaker, feeder.input_lengths, feeder.mel_targets, feeder.token_targets,
				targets_lengths=feeder.targets_lengths, global_step=global_step,
				is_training=True, split_infos=feeder.split_infos, reference_mels=feeder.reference_mels, reference_lengths=feeder.reference_lengths,Lf0=feeder.Lf0)
...
def model_test_mode(args, feeder, hparams, global_step):
	with tf.variable_scope('Tacotron_model', reuse=tf.AUTO_REUSE) as scope:
		...
		model = create_model(model_name or args.model, hparams)
		if hparams.predict_linear:
			model.initialize(feeder.eval_inputs, feeder.eval_speaker, feeder.eval_input_lengths, feeder.eval_mel_targets, feeder.eval_token_targets,
				linear_targets=feeder.eval_linear_targets, targets_lengths=feeder.eval_targets_lengths, global_step=global_step,
				is_training=False, is_evaluating=True, split_infos=feeder.eval_split_infos, reference_mels=feeder.eval_reference_mels, reference_lengths=feeder.eval_reference_lengths,Lf0=feeder.eval_Lf0)
		else:
			model.initialize(feeder.eval_inputs, feeder.eval_speaker, feeder.eval_input_lengths, feeder.eval_mel_targets, feeder.eval_token_targets,
				targets_lengths=feeder.eval_targets_lengths, global_step=global_step, is_training=False, is_evaluating=True, 
				split_infos=feeder.eval_split_infos, reference_mels=feeder.eval_reference_mels, reference_lengths=feeder.eval_reference_lengths,Lf0=feeder.eval_Lf0)
		model.add_loss(global_step)
		return model

def train(log_dir, args, hparams):
	save_dir = args.output_model_path
	plot_dir = os.path.join(log_dir, 'plots')
	wav_dir = os.path.join(log_dir, 'wavs')
	hparams_file = os.path.join(log_dir, '../hparams.txt')
	...
	input_path = args.tacotron_input
	...
	with tf.Session(config=config) as sess:
		try:
			...
			while not coord.should_stop() and step < args.tacotron_train_steps:
				...
				if hparams.use_style_encoder and (hparams.style_encoder_type=='vae' or hparams.style_encoder_type=='cvae'):
					step, loss, kl_loss, opt = sess.run([global_step, model.loss, model.kl_loss, model.optimize])
				else:
					step, loss, opt = sess.run([global_step, model.loss, model.optimize])
				time_window.append(time.time() - start_time)
				loss_window.append(loss)
				if hparams.use_style_encoder and (hparams.style_encoder_type=='vae' or hparams.style_encoder_type=='cvae'):
					message = 'Step {:7d} [{:.3f} sec/step, loss={:.5f}, kl_loss={:.5f},  avg_loss={:.5f}]'.format(
						step, time_window.average, loss, kl_loss, loss_window.average)
				else:
				...
				if loss > 100 or np.isnan(loss):
					log('Loss exploded to {} at step {}'.format(loss, step))
					raise Exception('Loss exploded')

				if step % args.summary_interval == 0:
					log('\nWriting summary at step {} {}'.format(step, loss))
				...
				if step % args.eval_interval == 0:
					...
					if hparams.predict_linear:
						for i in tqdm(range(feeder.test_steps)):
							eloss, before_loss, after_loss, stop_token_loss, linear_loss, mel_p, mel_t, t_len, align, lin_p, lin_t, speaker = sess.run([
								...
								eval_model.tower_linear_targets[0][0], eval_model.tower_speaker[0][0],
								])
						...
						for i in tqdm(range(feeder.test_steps)):
							eloss, before_loss, after_loss, stop_token_loss, mel_p, mel_t, t_len, align, speaker = sess.run([
								eval_model.tower_loss[0], eval_model.tower_before_loss[0], eval_model.tower_after_loss[0],
								eval_model.tower_stop_token_loss[0], eval_model.tower_mel_outputs[0][0], eval_model.tower_mel_targets[0][0],
								eval_model.tower_targets_lengths[0][0], eval_model.tower_alignments[0][0], eval_model.tower_speaker[0][0],
								])
							eval_losses.append(eloss)
							before_losses.append(before_loss)
							after_losses.append(after_loss)
					...
					plot.plot_alignment(align, os.path.join(eval_plot_dir, 'step-{}-eval-align.png'.format(step)),
						title='{}, {}, step={}, loss={:.5f}, speaker = {:d}'.format(args.model, time_string(), step, eval_loss, speaker),
						max_len=t_len // hparams.outputs_per_step)
					plot.plot_spectrogram(mel_p, os.path.join(eval_plot_dir, 'step-{}-eval-mel-spectrogram.png'.format(step)),
						title='{}, {}, step={}, loss={:.5f}, speaker = {:d}'.format(args.model, time_string(), step, eval_loss, speaker), target_spectrogram=mel_t,
						max_len=t_len)

					if hparams.predict_linear:
						plot.plot_spectrogram(lin_p, os.path.join(eval_plot_dir, 'step-{}-eval-linear-spectrogram.png'.format(step)),
							title='{}, {}, step={}, loss={:.5f}, speaker = {:d}'.format(args.model, time_string(), step, eval_loss, speaker), target_spectrogram=lin_t,
