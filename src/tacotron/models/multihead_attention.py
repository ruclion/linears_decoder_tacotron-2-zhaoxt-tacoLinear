...
class MultiheadAttention():
  ...
  def multi_head_attention(self):
    ...
    with tf.variable_scope("Multihead-attention"):
      q = tf.layers.conv1d(self.query, self.num_units, 1)  #[N, 1, 128]
      k =  tf.layers.conv1d(self.memory, self.num_units, 1) #[N, 10, 128]
      v = self.memory #[N, 10, 128]
      qs, ks, vs = self._split_heads(q, k, v) # qs[N, 4, 1, 32]  ks[N, 4, 10, 32] vs[N, 4, 10, 128]
